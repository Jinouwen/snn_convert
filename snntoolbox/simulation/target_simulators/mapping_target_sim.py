...
class SNN(AbstractSNN):
    ...
    def add_input_layer(self, input_shape):
        ...
        if self._poisson_input:
            ...
            self.layers.append(self.sim.NeuronGroup(
                ...
                dt=self._dt * self.sim.ms))
        self.layers[-1].add_attribute('count_base')
        self.layers[-1].count_base = self.count_base
        self.count_base += self.layers[-1].N
    ...
    def add_layer(self, layer):
        ...
        def add_connection(input):
            input_layer = self.layers[-2]
            id_str = input.name[:4]
            for it_layer in self.layers[:-1]:
                if id_str in it_layer.label:
                    input_layer = it_layer
                    print("find input layer")
                    break
            self.connections.append(self.sim.Synapses(
                input_layer, self.layers[-1], 'w:1', on_pre='',
                dt=self._dt * self.sim.ms))
    ...
    def build_convolution(self, layer, weights=None):
        ...
        # debug:
        print(connections[:, 0].max())
        print(self.connections[-1].source.N)
        print(self.connections[-1].target.N)
    ...
    def setup_layers(self, batch_shape):
        ...
        for layer in self.parsed_model.layers[1:]:
            ...
            for connection in self.connections:
                print('Saving')
                data_ij = np.zeros((len(connection), 2), dtype=int)
                data_ij[:, 0] = connection.i + connection.source.count_base
                data_ij[:, 1] = connection.j + connection.target.count_base
                # data_w[left:right] = connection.w
                np.savez(os.path.join(self.config.get('paths', 'path_wd'), '{:0>4d}data_ij.npz'.format(
                    self.filename_label)), data_ij)
    ...
    def save(self, path, filename):
        for connection in self.connections:
            self.connections_count += connection.N
        # data_w = np.zeros(self.connections_count)

        with open(os.path.join(path, 'N_M.txt'), 'w') as f:
            f.write('{}\n{}\n{}'.format(self.count_base, self.connections_count, self.filename_label))
        # np.savez(os.path.join(path, 'data_w.npz'), data_w)
